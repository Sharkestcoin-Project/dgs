...
@section('content')
<form action="{{ route('user.subscriptions.update', $subscription->id) }}" method="post" class="ajaxform_with_redirect product-wizerd" id="subscription-form">
    @csrf
    @method("PUT")
                ...
                <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="currency" id="currency" data-control="select2" required>
                                @foreach($currencies as $currency)
                                    <option value="{{ $currency->id }}" @selected($currency->id == $subscription->currency_id)>{{ $currency->code }} - {{ $currency->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="price" class="required">{{ __('Price') }}</label>
                            <input type="number" min="0" step="any" name="price" id="price" value="{{ $subscription->price }}" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label for="period" class="required">{{ __('Period') }}</label>
                            <select name="period" id="period" data-control="select2" required>
                                <option value="weekly" @selected($subscription->period == 'weekly')>{{ __('Weekly') }}</option>
                                <option value="monthly"  @selected($subscription->period == 'monthly')>{{ __('Monthly') }}</option>
                                <option value="yearly" @selected($subscription->period == 'yearly')>{{ __('Yearly') }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="tab">
                        <div class="form-group">
                            ...
                            <input type="text" name="return_url" id="return_url" class="form-control" placeholder="Ex: https://www.facebook.com/my-page" value="{{ $subscription->return_url }}">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <div class="justify-content-center">
                                    <img src="{{ $subscription->cover ? asset($subscription->cover) : asset('admin/img/img/placeholder.png') }}" alt="" id="cover_photo_parent_preview" class="card-img rounded-0" height="200">
                                </div>
                            </div>
                    ...
                    <div class="tab">
                        <div class="form-group">
                            ...
                            <textarea name="welcome_message" id="welcome_message" class="form-control" required>{{ $subscription->welcome_message ?? __('Thanks for purchasing our subscription') }}</textarea>
                        </div>
                                ...
                                @if(isset($subscription->features))
                                    @foreach($subscription->features ?? [] as $feature)
                                        <div class="input-group mb-3" data-repeater-item>
                                            <input type="text" name="title" class="form-control" value="{{ $feature['title'] }}" placeholder="{{ __('Feature title') }}" aria-label="{{ __('Features') }}">

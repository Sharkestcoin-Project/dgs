...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form class="ajaxform" method="post" action="{{ route('admin.settings.website.term.update') }}">
                        @csrf
                        @method('PUT')

                        <div class="form-group">
                            <label for="content" class="required">{{ __('Content') }}</label>
                            <textarea name="content" id="content" class="summernote" required>{{ $term['content'] ?? '' }}</textarea>
                        </div>

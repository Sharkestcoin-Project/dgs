...
class LanguageController extends Controller
{
    ...
    {
        $posts = Option::where('key', 'languages')->first()->value ?? null;
    ...
    public function store(Request $request)
    {
        ...
        $file = base_path('resources/lang/' . $request->input('language_code') . '.json');
        if (!file_exists($file)){
            copy(base_path('resources/lang/en.json'), $file);
        }

        $arr = [];
        $langlist = Option::where('key', 'languages')->first();
        if (!empty($langlist)) {
            foreach ($langlist->value as $key => $value) {
                $arr[$key] = $value;
            }
        }
    ...
    public function add_key(Request $request)
    {
        ...
        return response()->json(__('New Phrase Added Successfully'));
    }
}
